...
import { ArrowLeft, Factory, Bell } from "lucide-react";
...
export default function ProductionTopbar({ connected, pulse, lastUpdate, alertesCount, onToggleAlerts }) {
  ...
  return (
    <header className="flex items-center justify-between px-6 h-[56px] bg-background-surface/80 border-b border-border-subtle sticky top-0 z-40 backdrop-blur-xl ml-16">
      <div className="flex items-center gap-4">
        <button
          onClick={() => navigate("/")}
          className="p-1.5 rounded-lg bg-background-cards border border-border-subtle text-text-muted hover:text-accent-blue hover:border-accent-blue/30 transition-all group"
        ...
        <div className="flex flex-col">
          <div className="flex items-center gap-2">
            <Factory size={14} className="text-accent-blue" />
          ...
          <span className="text-[10px] text-text-muted font-medium uppercase tracking-widest">
            Suivi des Débits et Performances
          </span>
        </div>
      </div>
