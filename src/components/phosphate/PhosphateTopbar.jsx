...
export default function PhosphateTopbar({ connected, pulse, lastUpdate }) {
  ...
  return (
    <header className="flex items-center justify-between px-5 h-14 bg-slate-950/95 border-b border-white/5 sticky top-0 z-40 backdrop-blur">
      <div className="flex items-center gap-3">
        <button
          onClick={() => navigate("/")}
          className="w-7 h-7 bg-sky-400 rounded flex items-center justify-center font-black text-[#060d1a] text-sm hover:bg-sky-300 transition-colors"
          title="Retour au dashboard JFC1"
        >
          ←
        </button>
        <div className="w-7 h-7 bg-sky-400 rounded flex items-center justify-center font-black text-[#060d1a] text-sm">⊕</div>
        <span className="text-[13px] font-black text-sky-400 tracking-[.15em] uppercase">
          Phosphate — Analyse Qualité
        </span>
      </div>
