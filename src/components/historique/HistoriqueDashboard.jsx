...
import { fetchComparaison } from "../../services/historiqueService";
import { defaultDebut, defaultFin } from "../../utils/dateHelpers";
...
import { Calendar, Search, History, BarChart3, Database, AlertCircle, Loader2, GitCompare, Clock } from "lucide-react";

import ComparaisonPanel from "./ComparaisonPanel";
import ComparaisonChart from "./ComparaisonChart";
import ComparaisonStatsTable from "./ComparaisonStatsTable";
...
export default function HistoriqueDashboard() {
  const [viewMode, setViewMode] = useState('history'); // 'history' | 'comparison'
  ...
  // Comparison State
  const [debut1, setDebut1] = useState(() => defaultDebut(24 * 7));
  const [fin1, setFin1]     = useState(() => defaultFin());
  const [debut2, setDebut2] = useState(() => defaultDebut(24 * 14));
  const [fin2, setFin2]     = useState(() => defaultDebut(24 * 7));
  ...
  const setCompPreset = useCallback((days) => {
    const now = new Date();
    const p1Fin = new Date(now);
    const p1Debut = new Date(now.getTime() - days * 24 * 60 * 60 * 1000);
    const p2Fin = new Date(p1Debut);
    const p2Debut = new Date(p2Fin.getTime() - days * 24 * 60 * 60 * 1000);

    setDebut1(p1Debut.toISOString().slice(0, 16));
    setFin1(p1Fin.toISOString().slice(0, 16));
    setDebut2(p2Debut.toISOString().slice(0, 16));
    setFin2(p2Fin.toISOString().slice(0, 16));
  }, []);
      ...
      <div className="flex flex-1 overflow-hidden">
        ...
        <main className="flex-1 overflow-y-auto p-8 ml-16 bg-background-base">
          <div className="max-w-7xl mx-auto space-y-8 animate-fade-slide-up">
            ...
            {/* ── Sélecteur de Mode ── */}
            <div className="flex bg-background-cards p-1 rounded-xl border border-border-subtle w-fit">
              <button
            ...
            {viewMode === 'history' ? (
              ...
              <>
                {/* ── Mode Comparaison ── */}
                <ComparaisonPanel 
                  debut1={debut1} setDebut1={setDebut1}
                  fin1={fin1} setFin1={setFin1}
                  debut2={debut2} setDebut2={setDebut2}
                  fin2={fin2} setFin2={setFin2}
                  onCompare={handleCompare}
                  loading={compLoading}
                  setPreset={setCompPreset}
                />

                {compError && (
                  <div className="p-4 rounded-lg bg-accent-red/10 border border-accent-red/20 text-accent-red text-sm font-bold flex items-center gap-2">
                    <AlertCircle size={16} />
                    {compError}
                  </div>
                )}

                {compSearched && !compLoading && compData && (
                  <div className="space-y-8 animate-fade-slide-up">
                    <ComparaisonStatsTable 
                      statsP1={compData.statsP1}
                      statsP2={compData.statsP2}
                    />
                    <ComparaisonChart 
                      data={compData.chartData}
                      p1Label={compData.p1Label}
                      p2Label={compData.p2Label}
                    />
                  </div>
                )}

                {!compSearched && !compLoading && (
                  <div className="flex flex-col items-center justify-center py-32 text-center gap-4 bg-background-cards border border-border-subtle rounded-xl border-dashed">
                    <div className="p-4 rounded-full bg-accent-blue/5 text-accent-blue opacity-20">
                      <GitCompare size={64} />
                    </div>
                    <div>
                      <p className="text-lg font-bold text-text-primary">Analyse Comparative</p>
                      <p className="text-sm text-text-muted mt-1">Configurez deux périodes pour comparer leurs performances</p>
                    </div>
                  </div>
                )}
              </>
