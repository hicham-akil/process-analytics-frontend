...
import { ArrowLeft, ClipboardEdit, Bell } from "lucide-react";
...
export default function PerteTopbar({ connected, pulse, lastUpdate, alertesCount, onToggleAlerts }) {
  ...
  return (
    <header className="flex items-center justify-between px-6 h-[56px] bg-background-surface/80 border-b border-border-subtle sticky top-0 z-40 backdrop-blur-xl ml-16">
      <div className="flex items-center gap-4">
        <button
          onClick={() => navigate("/")}
          className="p-1.5 rounded-lg bg-background-cards border border-border-subtle text-text-muted hover:text-accent-green hover:border-accent-green/30 transition-all group"
        ...
        <div className="flex flex-col">
          <div className="flex items-center gap-2">
            <ClipboardEdit size={14} className="text-accent-green" />
